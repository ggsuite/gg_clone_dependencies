...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_clone_dependencies.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggCloneDependencies = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_clone_dependencies.dart',
      );

      expect(
        ggCloneDependencies,
        isNotNull,
        reason: 'Item for executing bin/gg_clone_dependencies.dart not found',
      );

      expect(ggCloneDependencies['name'], 'gg_clone_dependencies.dart');
      expect(ggCloneDependencies['type'], 'dart');
      expect(ggCloneDependencies['request'], 'launch');
